...
public class LsssMatrix {
    ...
    private LsssMatrix(LsssMatrixCell[][] matrix, String p){
    	policy=p;
        this.matrix = matrix;
    }
     ...
     */
    public static LsssMatrix createMatrixFromThresholdFormula(String policy, Pairing pub)
        ...
        return createMatrixFromGrowingMatrix(mainMatrix, pub, policy);
    }

    private static LsssMatrix createMatrixFromGrowingMatrix(GrowingMatrix<Element> matrix, Pairing pub, String policy) {
        ...
        return new LsssMatrix(newMatrix, policy);
    }
     ...
     * @throws ParseException Policy parsing failed
     */
    public static LsssMatrix createMatrixFromBooleanFormula(String policy, Pairing pub)
            throws ParseException {
        ...
        return createMatrixFromVectors(vectors, pub, policy);
    }
     ...
     * @return  Generated access matrix
     */
    private static LsssMatrix createMatrixFromVectors(Map<String, TreeNode> vectors, Pairing pub, String policy) {
        ...
        return new LsssMatrix(matrix, policy);
    }
    ...
    public ElementVector prod(ElementVector v, Pairing pairing) {
    	Field zR = pairing.getZr();
    	Element[] res= new Element[matrix.length];
    	for(int i=0; i<matrix.length; i++ ) {
    		res[i]=zR.newZeroElement();
    		for(int j=0; j<matrix[0].length;j++) {
    			Element a=zR.newElement(matrix[i][j].value);
    			a=a.mul(v.get(j));
    			res[i].add(a);
    		}
    		
    	}
    	return new ElementVector(res);
    }
    
    public static Element prod(ElementVector v, ElementVector v1, Pairing pairing) {
    	Field zR = pairing.getZr();
    	Element a=zR.newZeroElement();
    		for(int i=0; i<v.getDimension();i++) {
    			a.add(v.get(i).duplicate().mul(v1.get(i)));
